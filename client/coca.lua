...
function ProcessCoca()
	...
	TriggerServerEvent('viv_cocaine:processCoca')
	...
	while timeLeft > 0 do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(playerPed), Config.CircleZones.CocaProcessing.coords, false) > 4 then
			...
			TriggerServerEvent('viv_cocaine:cancelProcessing')
			break
		end
	end
	...
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			...
			if IsControlJustReleased(0, 38) and not isPickingUp then
				...
				ESX.TriggerServerCallback('viv_cocaine:canPickUp', function(canPickUp)
					if canPickUp then
						...
						TriggerServerEvent('viv_cocaine:pickedUpCoca')
					else
...
function SpawnCocaPlants()
	while cocaPlants < 15 do
		Citizen.Wait(0)
		local cocaCoords = GenerateCocaCoords()

		ESX.Game.SpawnLocalObject('prop_weed_01', cocaCoords, function(obj)
